...
*1.  *********************** Merge Women and Household memebrs datasets**********************
     *********This will introuduce household characteristics to the Women dataset************
	 
*a.Open secondary dataset, and sort by ID variable
use "C:\Users\alber\OneDrive\Documents\Github Projects\P1\ETPR71FL.DTA", clear
sort hhid  // sort by ID Variable

*b. Save temporary file of just the variables to merge in 
tempfile secondary_HHD 
save "`secondary_HHD'", replace  

*c. open primary file * i.e. Women dataset, and sort by ID Variable and merge
use "C:\Users\alber\OneDrive\Documents\Github Projects\P1\ETIR71FL.DTA", clear
gen hhid =substr(caseid,1,12) // changed ID variabe name to match ID variable name in second file 
sort hhid 
...
save "C:\Users\alber\OneDrive\Documents\Github Projects\P1\WM_HHM_Merged.DTA", replace

*2  ********** Setting survey parameters for complex survey design  and installing tabout for table production *********

ssc install tabout 

gen wt=hv005/1000000  //generating weight variable 
egen strata=group(v024 v025) 
* svyset [pw=x], psu(y) strata(z), where pw stands for probability weight, x = weight variable, y = cluster variable, z = strata variable. 
...
label variable pop "idividual women found"   // we created an individual value 1 foreach identified case and thats how we were able to calculate total population
gen totpop=sum(pop)
su totpop  // summing the total population of women

* b. Dissagregation by subgroups
...
replace room_crowd = 98 if room_crowd >= 98  // Accounting for Invalid entires and missing values, entires with 98.


* Now Calculating living space indicator 
gen living_space = 1
*As per standards, if persons per room is greator than 3, the no sufficient living space, hence, living1=0. I will move above code up to see if there is change.
...
svy: tab living_space m_agewm, per
       ...
       *2. Computing women living in overcrowded conditions by age group

svy: tab living_space v102, per

*6 ********************Regression and variable associations 

        *a. Studying the significance of association of residence, sex,education and wealth in relation to overcrowding
		 
* Here we analyse the results of the ch2 test from the cross tabulation below. 

tabout living_space hv025 [iw=wt] using "residencech2.xls",c(col) f(1) stats(chi2) svy nwt(wt) per pop replace // disagregation by residence
tabout living_space hv024 [iw=wt] using "regionch2.xls",c(col) f(1) stats(chi2) svy nwt(wt) per pop replace // disagregation by region
tabout living_space v149 [iw=wt] using "educationch2.xls",c(col) f(1) stats(chi2) svy nwt(wt) per pop replace // disagregation by education
tabout living_space hv270 [iw=wt] using "wealthch2.xls",c(col) f(1) stats(chi2) svy nwt(wt) per pop replace // disagregation by wealth
tabout living_space hv219 [iw=wt] using "hheadch2.xls",c(col) f(1) stats(chi2) svy nwt(wt) per pop replace // disagregation by hhold head

        *b. Logistic regression model to study weather wealth is explained by residence, education and sex:  
